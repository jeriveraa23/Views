...
AS SELECT article.id AS nota,
  ...
  WHERE dfv1.field_id = 23 AND (ticket_type.name::text <> ALL (ARRAY['PROYECTO'::character varying::text, 'PROCESO'::character varying::text, 'Unclassified'::character varying::text]))
UNION
 SELECT time_accounting.article_id AS nota,
  ...
  WHERE ticket_type.name::text = ANY (ARRAY['PROYECTO'::character varying::text, 'PROCESO'::character varying::text])
  ORDER BY 2;